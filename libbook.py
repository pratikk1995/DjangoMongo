...
def createAdminAccount():
    ...
    print('Enter security code number: ')
    seNumber = str(input(''))
    if seNumber == 'pratik':
            print('Enter your username : ')
            adminUserName1 = str(input(''))
            print('Enter your password : ')
            adminPassword1 = str(input(''))
            amidID = adminPassword1 + adminUserName1
            adminAccountCreateData = {'_id' : amidID, 'admin_user_name' : adminUserName1, 'admin_password' : adminPassword1}
            x = admin.insert_one(adminAccountCreateData)
            if x.inserted_id == amidID:
                print(adminUserName1, ', account successfully created.\n')
            adminSecurity()
    else:
        print('First collect security number from authority. Thanks!')
        exitApp()
    return
...
def welcfun():
     print('Welcome to Stackstorm Library')
     return
